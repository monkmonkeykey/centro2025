...
void draw() {
  ...
  for (int x = 0; x < webcam.width; x++) {
    for (int y = 0; y < webcam.height; y++) {
      ...
      // Creamos el color interpolado entre rojo oscuro y verde
      color rojo = color(120, 0, 0);
      color verde = color(0, 255, 0);
      float porcentaje = map(brillo, 0, 255, 0, 1);
      color gradiente = lerpColor(rojo, verde, porcentaje);
